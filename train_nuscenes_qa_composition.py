...
Tests point cloud + image composition for autonomous driving QA.
Dataset available on Hugging Face - NO REGISTRATION REQUIRED.

Usage:
    # Point cloud only
    python train_nuscenes_qa_composition.py --modality pointcloud

    # Image only
    python train_nuscenes_qa_composition.py --modality image

    # Both (composition)
    python train_nuscenes_qa_composition.py --modality both
"""
...
from safe.data.nuscenes_qa_dataset import NuScenesQADataset, collate_nuscenes_qa_batch
...
def compute_qa_metrics(predictions: List[str], references: List[List[str]]) -> Dict[str, float]:
    ...
    Returns:
        Dictionary with BLEU-1, BLEU-4, METEOR, Exact Match scores
    """
...
class NuScenesQACompositionModel(nn.Module):
    """
    Model for NuScenes-QA with optional modality composition.
...
def parse_args():
    parser = argparse.ArgumentParser(description="NuScenes-QA Composition Training")

    # Data
    parser.add_argument("--scene-type", type=str, default="day", choices=["day", "night", "both"])
    ...
    parser.add_argument("--camera-view", type=str, default="CAM_FRONT",
    ...
    parser.add_argument("--wandb-project", type=str, default="NuScenes-QA-Composition")
...
def main():
    ...
    print("=" * 60)
    print("NuScenes-QA Composition Training")
    print("=" * 60)
    ...
    # Create datasets
    print("\nLoading datasets from Hugging Face (no registration required!)...")

    if args.scene_type == "both":
        # Combine day and night scenes
        train_day = NuScenesQADataset(
            split="train",
            scene_type="day",
        ...
        train_dataset = ConcatDataset([train_day, train_night])

        val_day = NuScenesQADataset(
            split="validation",
            scene_type="day",
            modality=args.modality,
            num_points=args.num_points,
        ...
        val_dataset = ConcatDataset([val_day, val_night])
    else:
        train_dataset = NuScenesQADataset(
            split="train",
    ...
    train_loader = DataLoader(
        ...
        collate_fn=collate_nuscenes_qa_batch,
        pin_memory=True,
    )
    ...
    # Create model
    print("\nCreating model...")
    model = NuScenesQACompositionModel(
        modality=args.modality,
    ...
    elif hasattr(model, 'safe_model'):
        tokenizer = model.safe_model.base_vl.processor.tokenizer
    else:
    ...
    if len(trainable_params) == 0:
        print("WARNING: No trainable parameters! (Image-only mode uses frozen LLaVA)")

    optimizer = AdamW(trainable_params, lr=args.safe_lr) if trainable_params else None
    ...
    scheduler = LambdaLR(optimizer, lr_lambda) if optimizer else None

    # W&B
    if args.wandb and wandb is not None:
        wandb.init(
            project=args.wandb_project,
            name=args.wandb_run_name or f"nuscenes_qa_{args.modality}_{args.scene_type}",
    ...
    for epoch in range(args.num_epochs):
        ...
        if optimizer:
            ...
            print(f"\nEpoch {epoch+1} - Skipping training (image-only mode)")

        # Evaluate
        if (epoch + 1) % args.eval_every == 0:
            print(f"\nEvaluating...")
            eval_metrics, preds, refs = evaluate(
                model, val_loader, args.device, args, tokenizer,
                max_samples=args.max_eval_samples,
            )

            print(f"  BLEU-1: {eval_metrics['bleu1']:.2f}%")
            print(f"  BLEU-4: {eval_metrics['bleu4']:.2f}%")
            print(f"  METEOR: {eval_metrics['meteor']:.2f}%")
            print(f"  Exact Match: {eval_metrics['exact_match']:.2f}%")

            if args.wandb and wandb is not None:
                wandb.log({
                    "epoch": epoch + 1,
                    "eval/bleu1": eval_metrics["bleu1"],
                    "eval/bleu4": eval_metrics["bleu4"],
                    "eval/meteor": eval_metrics["meteor"],
                    "eval/exact_match": eval_metrics["exact_match"],
                })

            # Save best model
            if eval_metrics["exact_match"] > best_exact_match:
                best_exact_match = eval_metrics["exact_match"]
                save_path = Path(args.output_dir) / "best_model.pt"
                if hasattr(model, 'safe_model'):
                    torch.save(model.safe_model.state_dict(), save_path)
                    print(f"  Saved best model to {save_path}")

    # Save final model
    final_path = Path(args.output_dir) / "final_model.pt"
    if hasattr(model, 'safe_model'):
        torch.save(model.safe_model.state_dict(), final_path)
    ...
    print(f"\nTraining complete! Best Exact Match: {best_exact_match:.2f}%")
