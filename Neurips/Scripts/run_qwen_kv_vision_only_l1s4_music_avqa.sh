#!/bin/bash
#SBATCH --job-name=qwen_kv_vis_l1s4
#SBATCH --output=logs/qwen_kv_vis_l1s4_%j.out
#SBATCH --error=logs/qwen_kv_vis_l1s4_%j.err
#SBATCH --time=48:00:00
#SBATCH --mem=96G
#SBATCH --cpus-per-task=8
#SBATCH -p gpu

# Vision-only KV aug: 8 layers starting at 1, stride 4
...
export OUTPUT_DIR="${OUTPUT_DIR:-$SAFE_ROOT/checkpoints/qwen_kv_vision_only_l1s4}"
...
export WANDB_RUN_NAME="${WANDB_RUN_NAME:-qwen_kv_vis_l1s4_${SLURM_JOB_ID:-local}}"
export WANDB_TAGS="${WANDB_TAGS:-neurips,qwen,kv,music_avqa,vision_only,l1s4,8layers}"
