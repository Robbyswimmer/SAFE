#!/bin/bash
#SBATCH --job-name=qwen_kv_audio_after_vision
#SBATCH --output=logs/qwen_kv_audio_after_vision_%j.out
#SBATCH --error=logs/qwen_kv_audio_after_vision_%j.err
...
export VISION_CKPT="${VISION_CKPT:-$SAFE_ROOT/checkpoints/qwen_kv_vision_only/best_model.pt}"
export MODEL_CONFIG="${MODEL_CONFIG:-composition_kv_qwen}"
export OUTPUT_DIR="${OUTPUT_DIR:-$SAFE_ROOT/checkpoints/qwen_kv_audio_after_vision}"
...
export WANDB_RUN_NAME="${WANDB_RUN_NAME:-qwen_kv_audio_after_vision_${SLURM_JOB_ID:-local}}"
export WANDB_TAGS="${WANDB_TAGS:-neurips,qwen,kv,music_avqa,audio_after_vision,sequential}"
