#!/bin/bash
#SBATCH --job-name=rkca
#SBATCH --output=logs/rkca_%j.out
#SBATCH --error=logs/rkca_%j.err
...
# Audio and vision projectors trained independently (interleaved), then
# compose at eval via simple concatenation — no joint data, no regularization.
...
OUTPUT_DIR=${OUTPUT_DIR:-outputs/rkca_avqa}
...
WANDB_RUN_NAME=${WANDB_RUN_NAME:-rkca_${SLURM_JOB_ID:-local}}
WANDB_TAGS=${WANDB_TAGS:-rkca,composition,concat,interleaved}
