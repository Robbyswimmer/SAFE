#!/bin/bash
#SBATCH --job-name=d3-vision-4b
#SBATCH --output=logs/d3_vision_4b_%j.out
#SBATCH --error=logs/d3_vision_4b_%j.err
...
# D3: Vision-only adapter on Qwen3-4B
# Independent training — only vision adapter learns.
...
OUTPUT_DIR=${OUTPUT_DIR:-checkpoints/d3_vision_4b}
BATCH_SIZE=${BATCH_SIZE:-1}
EPOCHS=${EPOCHS:-10}
LR=${LR:-5e-5}
NUM_AUDIO_TOKENS=${NUM_AUDIO_TOKENS:-8}
TRAIN_MODALITY=image
EVAL_MODALITIES=${EVAL_MODALITIES:-text,image}
SEED=${SEED:-42}
WANDB=${WANDB:-1}
WANDB_PROJECT=${WANDB_PROJECT:-SAFE-Composition}
WANDB_RUN_NAME=${WANDB_RUN_NAME:-d3_vision_4b_s${SEED}_${SLURM_JOB_ID:-local}}
WANDB_TAGS=${WANDB_TAGS:-diagnostic,d3,vision_only,4b,independent}
