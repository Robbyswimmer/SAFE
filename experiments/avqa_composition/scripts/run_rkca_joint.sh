#!/bin/bash
#SBATCH --job-name=rkca_joint
#SBATCH --output=logs/rkca_joint_%j.out
#SBATCH --error=logs/rkca_joint_%j.err
...
# RKCA Joint experiment: Concat audio on InternVL 8B with native vision always active.
# Every training batch includes both audio and images. The audio projector learns in
# the context of visual features — the LLM sees vision + audio simultaneously during
# training, so the CE loss teaches the audio projector to complement (not duplicate)
...
OUTPUT_DIR=${OUTPUT_DIR:-outputs/rkca_joint_avqa}
...
WANDB_RUN_NAME=${WANDB_RUN_NAME:-rkca_joint_${SLURM_JOB_ID:-local}}
WANDB_TAGS=${WANDB_TAGS:-rkca,joint,composition,concat,internvl}
