#!/bin/bash
#SBATCH --job-name=comp-audio
#SBATCH --output=logs/composition_audio_%j.out
#SBATCH --error=logs/composition_audio_%j.err
...
# Composition experiment: Audio-only SAFE adapters on Qwen3-8B
# Uses composition config so fusion adapter has both audio and vision slots,
# but only audio trains here.
...
EVAL_MODALITIES=${EVAL_MODALITIES:-audio}
WANDB=${WANDB:-1}
WANDB_PROJECT=${WANDB_PROJECT:-SAFE-Composition}
WANDB_RUN_NAME=${WANDB_RUN_NAME:-composition_audio_${SLURM_JOB_ID:-local}}
WANDB_TAGS=${WANDB_TAGS:-composition,audio_only}
