...
J1=$(sbatch --parsable --gres=gpu:1 \
  --export=ALL,SAFE_ROOT="$SAFE_ROOT",MODEL_CONFIG="$MODEL_CONFIG",OUTPUT_DIR=checkpoints/composition_level1_calibration,SEED="$SEED",FUSION_GATE="$FUSION_GATE",SLIM_PROJECTOR=0,INIT_AUDIO_CKPT="$BASE_CKPT",INIT_VISION_CKPT="$BASE_CKPT",EPOCHS=2,MAX_SAMPLES="$LEVEL1_MAX_SAMPLES",LEARNED_GATE=1,LEARNED_GATE_INIT=0.0,TRAIN_GATES_ONLY=1,COMPAT_NOHARM_ENABLE=1,COMPAT_NOHARM_LAMBDA=0.05,COMPAT_NOHARM_USE_BEST_SINGLE=1,COMPAT_POE_ENABLE=1,COMPAT_POE_LAMBDA=0.05,COMPAT_POE_WEIGHT_TEMP=0.5,COMPAT_POE_LOSS_TYPE=kl,COMPAT_POE_LOGIT_TEMP=1.0,COMPAT_ROUTING_ENABLE=1,COMPAT_ROUTING_MIN_SCALE=0.25,COMPAT_ROUTING_MAX_SCALE=1.0,WANDB_RUN_NAME=level1_calibration_${SEED},WANDB_TAGS=composition,level1,calibration,gates_only \
...
J2=$(sbatch --parsable --gres=gpu:1 \
  --export=ALL,SAFE_ROOT="$SAFE_ROOT",MODEL_CONFIG="$MODEL_CONFIG",OUTPUT_DIR=checkpoints/composition_level2_full,SEED="$SEED",FUSION_GATE="$FUSION_GATE",SLIM_PROJECTOR=0,INIT_AUDIO_CKPT="$BASE_CKPT",INIT_VISION_CKPT="$BASE_CKPT",EPOCHS=3,MAX_SAMPLES="$LEVEL2_MAX_SAMPLES",COMPAT_REG_ENABLE=1,COMPAT_REG_LAMBDA=0.05,COMPAT_REG_RANK=8,COMPAT_REG_AUDIO_SAMPLES=256,COMPAT_REG_WEIGHT_BY_SHIFT_NORM=1,COMPAT_ADD_REG_ENABLE=1,COMPAT_ADD_REG_LAMBDA=0.01,COMPAT_ADD_REG_EVERY=200,COMPAT_ADD_REG_NORMALIZE=1,COMPAT_NOHARM_ENABLE=1,COMPAT_NOHARM_LAMBDA=0.02,COMPAT_NOHARM_USE_BEST_SINGLE=1,COMPAT_POE_ENABLE=1,COMPAT_POE_LAMBDA=0.02,COMPAT_POE_WEIGHT_TEMP=0.5,COMPAT_POE_LOSS_TYPE=kl,COMPAT_POE_LOGIT_TEMP=1.0,COMPAT_ROUTING_ENABLE=1,COMPAT_ROUTING_MIN_SCALE=0.25,COMPAT_ROUTING_MAX_SCALE=1.0,WANDB_RUN_NAME=level2_full_${SEED},WANDB_TAGS=composition,level2,full_objective \
  experiments/avqa_composition/scripts/train_composition_interleaved.sh)
