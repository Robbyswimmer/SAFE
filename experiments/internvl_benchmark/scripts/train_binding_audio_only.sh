#!/bin/bash
#SBATCH --job-name=internvl-bind
#SBATCH --output=logs/internvl_binding_%j.out
#SBATCH --error=logs/internvl_binding_%j.err
...
#SBATCH --gres=gpu:1
#
# InternVL 3.5-8B Modality Binding Experiment
#
# Hypothesis: InternVL already understands vision natively. If we train
# ONLY an audio adapter (projector + fusion) on audio-text QA data,
# does InternVL's frozen vision compose with audio at eval time?
#
# Protocol:
#   - Train: audio adapter only on MUSIC-AVQA audio-text pairs (NO images)
#   - Eval:  text-only, audio-only, image-only (built-in vision), audio+image
#
# If audio+image > audio-only at eval, the frozen backbone is binding
# modalities without explicit joint training.
#
# Uses bottleneck_dim=756 projector (756/4096 = 18.5% ratio).
#
# Usage:
#   sbatch experiments/internvl_benchmark/scripts/train_binding_audio_only.sh
...
OUTPUT_DIR=${OUTPUT_DIR:-$SAFE_ROOT/experiments/internvl_benchmark/outputs/binding_audio_only}
...
WANDB_NAME="internvl_binding_audio_${SLURM_JOB_ID:-local}"
WANDB_TAGS=${WANDB_TAGS:-"internvl,binding,audio-only,bn756"}

echo "========================================"
echo "InternVL 3.5-8B Modality Binding Experiment"
echo "========================================"
...
echo ""
echo "========================================"
echo "Binding Experiment Complete!"
echo "========================================"
...
echo "  - image-only: InternVL built-in vision (no audio training)"
echo "  - both:       BINDING TEST - does audio+vision > audio alone?"
echo "========================================"
