...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        **kwargs,  # Accept and ignore extra kwargs to prevent PEFT errors
        ...
        # Residual connection (optionally followed by layer norm)
        hidden_states = hidden_states.to(delta.dtype)
...
class LoRAFusionAdapter(nn.Module):
    ...
    def forward(
        ...
        # Apply gating on the residual update so gate=0.0 is an exact no-op
        if gate != 1.0:
            delta = fused_states - hidden_states
            fused_states = hidden_states + gate * delta
